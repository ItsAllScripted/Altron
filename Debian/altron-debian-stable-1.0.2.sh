...
function logger {
  ...
  update_time printf "\33[2K$MESSAGE\r"
}
...
for i in "${!ADMINS[@]}"; do
  ...
  echo "  - ${ADMINS[$i]}: ${user_password}" >> $PASSWORD_FILE
  yes $user_password | sudo passwd ${ADMINS[$i]} >> $LOGFILE 2>&1
...
for i in "${!USERS[@]}"; do
  random_password "16"; user_password=$password
  echo "  - ${USERS[$i]}: ${user_password}" >> $PASSWORD_FILE
  yes $user_password | sudo passwd ${USERS[$i]} >> $LOGFILE 2>&1
...
if [ $FNRET != 0 ]; then
  logger "$NTP_CONF_DEFAULT_PATTERN not found in $NTP_CONF_FILE, adding it\n"
...
if [ $FNRET != 0 ]; then
  logger "$NTP_INIT_PATTERN not found in $NTP_INIT_FILE, adding it\n"
...
if [ $FNRET = 0 ]; then
  ...
  if [ $FNRET != 0 ]; then
    logger "$RSYNC_DEFAULT_PATTERN not found in $RSYNC_DEFAULT_FILE, adding it\n"
...
echo "Editing automatically gids may seriously harm your system, report only here"
...
khunter --propupd >> /dev/null
khunter --check --nocolors --skip-keypress >> $RKHUNTER_FILE 2>&1
